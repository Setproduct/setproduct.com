...
- Узкие GFM-таблицы (2-3 коротких колонки, например "Criteria | What it
  measures") рендерятся с пустым белым полем справа. Причина: глобальное
  правило в `styles/globals.css` (`.rich-text-18 table`) ставит таблице
  `display: block; overflow-x: auto`, чтобы широкие таблицы могли
  скроллиться по горизонтали на мобильных. Побочный эффект — узкая
  таблица не растягивается на всю ширину контейнера, а остаётся по
  ширине своего контента, оставляя пустоту справа.
  Фикс: оборачивать именно узкие/короткие таблицы в
  `<div class="blog-fit-table">...</div>` (обязательно с пустой строкой
  до и после самой таблицы markdown, иначе MDX не распознает markdown
  внутри HTML-блока). Класс `.rich-text-18 .blog-fit-table table`
  переключает таблицу обратно на `display: table; width: 100%`, и она
  растягивается на всю ширину без пустого поля.
  Широкие таблицы (5+ колонок, например сравнительные таблицы платформ)
  в обёртке не нуждаются — они и так заполняют ширину или скроллятся
  по горизонтали на мобильных, что и является ожидаемым поведением.
  Примеры использования обёртки — `content/blog/badge-ui-design.mdx`,
  `content/blog/date-picker-ui-design.mdx`,
  `content/blog/data-table-ui-design.mdx`.
...
### КАТЕГОРИЧЕСКИЕ ЗАПРЕТЫ ДЛЯ ЛЮБОГО ТЕКСТА БЛОГА

Это правила уровня «hard limit, zero exceptions». Они НЕ отменяются
форматом из брифа, шаблоном заголовка, пожеланием «сделай красиво»
или удобством формулировки. При конфликте «шаблон из задачи vs
правило ниже» — побеждает правило; если это меняет структуру,
сначала спроси автора, потом пиши.

1. **Em-dash (—): максимум 2 на статью.** Считается по всему файлу
   целиком, включая frontmatter, title, subtitle, description, FAQ и
   заголовки. 16 заголовков вида `### N. Name — phase` — это 16
   нарушений, а не «формат из брифа». Вместо тире — запятые, скобки,
   двоеточие или новое предложение.
2. **Двоеточие в title/metaTitle ЗАПРЕЩЕНО.** Источник:
   `.roo/skills/seo/SKILL.md`. Двоеточие режет тайтл в SERP и роняет CTR.
   Это относится и к H2/H3, если заголовок по сути является тайтлом раздела.
3. **Трихолоны (X, Y and Z / X, Y, Z): максимум 2 на статью.**
   Считаются и в теле, и во frontmatter. Перед сдачей перечислить все
   найденные и переписать лишние в пары, «X, затем Y» или отдельные
   предложения.
4. **Запрещённые конструкции:** «Not X, but Y», любое предложение или
   фрагмент, начинающийся с «Not», «It's not just X, it's Y»,
   «Whether you're X or Y», «From X to Y» как структурный шаблон.
5. **Банклише из списка скилла блога** — удалять по факту обнаружения.
6. **Sentence case** — см. раздел выше, без исключений.

### ОБЯЗАТЕЛЬНАЯ САМОПРОВЕРКА ПЕРЕД СДАЧЕЙ ЛЮБОГО MDX-ТЕКСТА

Ни один отчёт по статье (attempt_completion) не отправляется, пока
не выполнен и не показан результат этого прогона. Это не
рекомендация, а блокирующий шаг:
...
Пороги, при которых файл НЕ сдаётся: em-dash > 2; двоеточие в любом
H2/H3 (кроме случаев, когда это внутри строки, а не заголовок);
title > 65 символов или содержит «:»; трихолонов > 2; любое
совпадение «Not X, but Y»; нечистый `npm run lint`.

Отдельно и обязательно: перед сдачей пересказать в отчёте фактические
цифры прогона (em-dash, трихолоны, слова, длина title/description),
а не формулировку «правила соблюдены».

### ПОРЯДОК РАБОТЫ С КОНТЕНТОМ (обязательный STEP 0)

Перед написанием или правкой любого текста блога:
1. Прочитать целиком все три скилла: seo, blog-writing, copywriter.
2. Выписать в 5 строк лимиты из них (em-dash, трихолоны, запрещённые
   конструкции, требования к title/description).
3. Только после этого приступать к тексту. Пропуск этого шага —
   основная причина массовых правок задним числом и сожжённого compute.
4. Любое отклонение от брифа, вынужденное правилами, фиксировать в
   отчёте; любое отклонение от правил — недопустимо.
...
|---|---|---|---|
| `spacer-4` | 4em | 4em | Заголовок карточки/строки → её описание |
...
### Скругления картинок и карточек (обязательное правило)

**Правило:** скругление углов у картинок, превью и карточек задаётся только классами шкалы `radius-*` (или `em`-значением из этой же шкалы в CSS). Tailwind-классы `rounded-*` для картинок и карточек **не используются**: они в `rem` и не масштабируются вместе с вёрсткой, так же как `mb-*` против `spacer-*`.

**Шкала** (определения в [`public/css/setproduct.webflow.shared.css`](public/css/setproduct.webflow.shared.css:2644)):

| Класс | Desktop (≥480px) | Мобильный (≤479px) | Применение |
|---|---|---|---|
| `radius-24` | 24em | 16em | Большие картинки и карточки: карточки шаблонов и постов, обложки, галереи, картинки в теле статьи, карусели |
| `radius-16` | 16em | 12em | Средние: миниатюры в мега-меню, низкие карточки, «Top results» на /search |
| `radius-12` | 12em | 8em | Маленькие превью 64–128px: миниатюры в строках результатов /search |
| `radius-8` | 8em | 8em | Микро: бейджи, крошечные превью |
| `radius-full` | 100em | 100em | Аватары, «таблетки» (для круга у квадрата можно `50%`) |

**Как выбирать ступень:**
1. **Радиус ≈ 8–12% от короткой стороны картинки**, округлить до ближайшей ступени. Картинка 96px высотой → ~12 → `radius-12`; карточка 385px → `radius-24`.
2. **Правило вложенности:** внешний радиус = внутренний радиус + отступ между ними. Картинка `radius-16` внутри карточки с `padding: 8em` → у карточки `radius-24`. Если картинка прижата к краю карточки без отступа (как в `.blog_list-item`), скругляется контейнер с `overflow: hidden`, а у картинки радиус 0.
3. Нужного значения нет — берём ближайшую ступень. Новые значения (14em, 10px и т.п.) не вводим.

**Ловушка em внутри текста.** `em` считается от `font-size` самого элемента. Это ≈1px только там, где `font-size` унаследован от `body`. Внутри `.rich-text-18 p` (`font-size: 18em`) значение `border-radius: 24em` превращается в 24 × 18 = 432px, и картинка в статье становится «таблеткой». Поэтому классы `radius-*` написаны через `calc(N * var(--em-px))`: `--em-px` повторяет `font-size` у `body` на всех брейкпоинтах и от шрифта элемента не зависит. Если пишете радиус прямо в CSS для элемента, у которого (или у предка которого) свой `font-size`, используйте `calc(N * var(--em-px))`, а не `Nem`.

### Ховер карточек с картинкой (обязательное правило)

**Правило:** у любой кликабельной карточки с картинкой (превью в мега-меню, карточки на /search, в блоге, в листингах шаблонов и freebies) ховер делается только общим паттерном `hover-lift` из [`styles/globals.css`](styles/globals.css). Свои Tailwind-ховеры на карточках (`group-hover:scale-*`, `hover:-translate-y-*`, `hover:shadow-*`, `transition-transform` на картинке) **не пишем**: они в `rem`, с другими кривыми и таймингами и ломают единое ощущение.

**Классы:**

| Класс | Куда ставить | Что делает |
|---|---|---|
| `hover-lift` | Корень карточки (ссылка или обёртка) | Триггер ховера и фокуса, задаёт переменные подъёма, тени и зума |
| `hover-lift is-large` | Корень большой карточки | Усиленный подъём и тень, мягкий зум |
| `hover-lift-media` | Рамка с картинкой или вся карточка с бордером | Поднимается и отбрасывает тень, обрезает зум (`overflow: hidden`) |
| `hover-lift-title` | Заголовок карточки | Плавно окрашивается в `var(--primary)` |

Картинка отдельного класса не требует: правило `.hover-lift-media img` находит любой `img` внутри media.

**Размеры:**

| Вариант | Подъём | Тень | Зум картинки | Когда |
|---|---|---|---|---|
| по умолчанию | 3em | `0 14em 28em -10em` | 1.06 | Короткая сторона картинки < 240em: мега-меню, строки и «Top results» на /search |
| `is-large` | 6em | `0 24em 48em -16em` | 1.03 | Короткая сторона ≥ 240em: карточки блога, шаблонов, freebies, бандлов, прайсинга |

Тайминги общие: подъём и тень 260ms `cubic-bezier(.22,1,.36,1)`, зум 420ms, цвет заголовка 200ms. Ховер работает только при `(hover: hover) and (pointer: fine)`. Тот же эффект срабатывает при `:focus-visible` (навигация с клавиатуры). При `prefers-reduced-motion` остаётся только тень, без движения.

**Как применять:**
1. Если у карточки есть обёртка с бордером, скруглением и `overflow: hidden` (как `.blog_list-item-wr`), `hover-lift-media` ставится на эту обёртку, и поднимается вся карточка. Внутренней рамке картинки добавляем `overflow-hidden`, чтобы зум не вылезал на текст.
2. Если бордера нет и картинка стоит отдельной рамкой (как `.template-list-item-img-wr`), `hover-lift-media` ставится на рамку картинки.
3. **Проверить предков на `overflow: hidden`** и `overflow-x: auto`: они обрезают подъём и тень. Лечится точечно: `overflow: visible` на десктопе (см. `.main_blog-liist2-wr` в [`BlogPostsHome.module.css`](components/sections/BlogPostsHome.module.css)) или запас `pt/pb` с отрицательным `mt/mb` на горизонтальном треке (см. [`FreebiesShowcase`](components/sections/FreebiesShowcase.tsx)).
